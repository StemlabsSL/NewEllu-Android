...
import com.github.ik024.calendar_lib.adapters.YearItemAdapter;
 ...
 */
public class YearView extends LinearLayout implements MonthViewClickListeners, MyOnItemClickListener, YearViewClickListeners {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            monthTextColor = ContextCompat.getColor(context, R.color.colorAccent);
            daysOfWeekTextColor = ContextCompat.getColor(context, R.color.colorAccent);
        ...
        rvYear = (RecyclerView) view.findViewById(R.id.rvYear);
    ...
    private void setAdapter() {
        rvYear.setLayoutManager(new GridLayoutManager(getContext(), 2));

        ArrayList<YearItem> list2 = new ArrayList<YearItem>();


        for (int i = mDisplayYear; i < mDisplayYear + 20; i++) {


            if (i == mDisplayYear) {
                for (int j = mCurrentMonth; j < 12; j++) {
                    list2.add(new YearItem(j, i));
                }
            } else {
                for (int j = 0; j < 12; j++) {
                    list2.add(new YearItem(j, i));
        ...
        yearItemAdapter = new YearItemAdapter(list2, getContext(), this, rvYear);
        rvYear.setAdapter(yearItemAdapter);
    }
    ...
    @Override
    public void onItemClicked(int position, RecyclerView view, int year, int month) {

//        if (mYearClickListeners != null) {
//            mYearClickListeners.dateClicked(year, month, 1);
//        }
    }
