...
//    HashMap<String,Integer> drawingStr
    ...
    private void initLoopView(Context context, AttributeSet attributeset) {
        ...
        outerTextColor = typedArray.getInteger(R.styleable.androidWheelView_awv_outerTextColor, 0xfF8F8F8f);
...
//            drawingStrings = new String[itemsVisibleCount];
     ...
     * @param size
     */
    public final void setTextSize(float size,float size2) {
        if (size > 0.0F) {
        ...
        if(size2 > 0.0f)
        {
            paintCenterText.setTextSize(size2);
        }
    }
        ...
        while (i < itemsVisibleCount) {
            ...
            if (radian >= Math.PI || radian <= 0) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    int yPos = 0;
                    yPos += (Math.abs(firstLineY + secondLineY)) / 10;
                    paintCenterText.getTextBounds(drawingStrings.get(i).string, 0,drawingStrings.get(i).string.length(), tempRect);
                    canvas.clipRect(0, 0, measuredWidth, (int) (itemHeight));canvas.drawText(drawingStrings.get(i).string, getTextX(drawingStrings.get(i).string, paintCenterText, tempRect),yPos, paintCenterText);
